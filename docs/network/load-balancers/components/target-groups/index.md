...
![Target Groups Tab](../../../networking-images/target_groups_1_small.png)
...
There are 3 ways to balance traffic between the target servers, known as the load balancing method:
...
These timeout options will be left at the ANS default setting unless you change them, except for tunnel which will not be set unless you specify a value.

- **HTTP Request** - The maximum amount of time to wait for a complete HTTP request from the client. The ANS default is currently 10 seconds.
- **Check** - Set additional check timeout, but only after a connection has been already established. The ANS default is currently 10 seconds.
- **Tunnel** - Sets the maximum inactivity time on the client and server-side for tunnels (e.g. WebSockets, CONNECT).

## Saving and deploying

Once you've finished filling in the target group form, click `Create Target Group` to add the target group to the load balancer. Once you've finished setting up all your target groups, make sure you [deploy your changes](../../deploying-changes/).
