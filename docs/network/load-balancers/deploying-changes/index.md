...
![Deployment Alert](../../networking-images/deployment_alert_small.png)
...
If no changes are detected between the current configuration deployed to the load balancer and the current configuration within ANS Glass, the deployment screen won't allow you to attempt a deployment. If you believe this is an error you should contact support.

**Ready to deploy**

If you've made changes since the last deployment and the configuration is valid then you will get a blue "Deploy Now" button. Clicking this will deploy the configuration you have created in ANS Glass to the load balancer so that your changes start to take effect.

![Ready to deploy](../../networking-images/deployment_validation_passed_small.png)
...
If you are unsure what the configuration validator is asking you to do, you can always create a PSS request to speak to a member of our support team\*.

Once you've fixed all the issues and gone back to the deployments screen it will change to the "ready to deploy" screen.

![Configuration error](../../networking-images/deployment_validation_failed_small.png)

\* _Not available for eCloud VPC load balancers unless you have the optional support add-on_
