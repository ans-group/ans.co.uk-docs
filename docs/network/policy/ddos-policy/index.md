...
We have an anti-DDoS monitoring service deployed across our entire network. The threshold is currently set to 200,000 packets per second (200k pps) or 3072Mbps of traffic. If the traffic pattern for any one IP address is exceeding those thresholds, the IP address will be automatically black-holed. If after 30 minutes the attack is still ongoing, the IP address will remain black-holed until a further check 30 minutes after that, and so on.
