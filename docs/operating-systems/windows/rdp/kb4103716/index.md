---
sidebar_position: 6
sidebar_label: 'KB4103716: CredSSP updates - "An authentication error has occurred"'
title: 'KB4103716: CredSSP updates - "An authentication error has occurred"'
...
Following this update some ANS customers may find they are unable to access their servers with us and receive the following error message when attempting to connect via Remote Desktop:
...
In almost all cases, this error will occur because the above update has been applied on your client computer but _not_ the server you are attempting to connect to.
...
The same policy is present on patched Windows servers, and connections from _unpatched_ clients will also be prevented if the server's 'Protection Level' is set to `Force Updated Clients`. In this instance, the following error will be presented on the client computer:
...
![Encryption Oracle Remediation Policy](../../../operating-systems-images/enable_vuln_protection_level_highlight.png)

:::note
Changes to this policy require a reboot to take effect.
:::
...
If you don't have access to the updated policy (`.admx`) or resource (`.amdl`) files and would like to utilise these in a ANS environment, please contact ANS Support by [raising a ticket in ANS Glass](https://ans.glass/support-and-help/incidents) or calling 0800 230 0032.
