...
As most ANS servers are either CentOS/Red Hat or Ubuntu/Debian, this article will only cover two of the most popular Linux package managers, `yum` and `apt`.
...
It also highlights the package's name in a different colour based on its status.
...
In the above example, if you had finished with the packages you installed in transaction 4, you could then remove those by running `yum history undo 4`. If you wished to rollback your server to transaction 2, you could run `yum history rollback 2` which would remove packages installed in transactions 3 and 4.
