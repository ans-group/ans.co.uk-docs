---
sidebar_position: 8
sidebar_label: "Sender Policy Framework (SPF) records"
title: Sender Policy Framework (SPF) records
...
Once you have a suitable SPF record, please refer to this [guide on how to add your record to SafeDNS](../../domains-and-dns-management/safedns/spf).
...
| `ip4`     | Match an IPv4 address or subnet /32 subnet is the default if not given.                                                      | `ip4:46.37.163.252/29`                    |
| `ip6`     | Match an IPv6 address or subnet. /128 subnet is the default if not given.                                                    | `ip6:2001:db8:85a3::8a2e:370:7334/64`     |
| `a`       | Match the A records for example.com Optionally includes a subnet range. If no domain is specified, match the current domain. | `a:example.com` `a:example.com/29` `a`    |
| `mx`      | Match MX records for example.com. Optionally includes a subnet range. If no domain is specified, match the current domain.   | `mx:example.com` `mx:example.com/29` `mx` |
| `ptr`     | Match PTR (Also called rDNS) records. Not generally used as it results in long DNS lookups.                                  | `ptr:example.com`                         |
| `exists`  | Match if the domain given returns an A record. Only really useful in macros.                                                 | `exists:example.com`                      |
| `include` | Include another domain's SPF record for matches. Used for managing trust relationships between domains.                      | `include:example.com`                     |
...
| --------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `\+`      | Pass               | This is the default action if a mechanism doesn't have a qualifier set. If the mechanism matches, it means that host is allowed to send mail and mail should be accepted. |
