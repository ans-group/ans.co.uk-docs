...
More details on the project can be seen on the official repository [https://github.com/acmesh-official/acme.sh](https://github.com/acmesh-official/acme.sh).
...
:::note
Replace `<webserver>` with the web server you are using. This will likely be `NGINX` or `Apache`. This ensures the service is restarted when the certificate is renewed. Apache’s service name also changes depending on the OS it is installed on. For Debian variants (e.g. Ubuntu) this is `apache2`. For RHEL variants (e.g. Alma) this is `httpd`.
:::
...
`acme.sh` also has an `NGINX` mode. This will only work if you are _currently_ running `NGINX` on port **80**
...
`acme.sh` also has an `Apache` mode. This will only work if you are _currently_ running `Apache` on port **80**
