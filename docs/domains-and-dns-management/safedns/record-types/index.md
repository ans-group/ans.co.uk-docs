...
- `ALIAS` Record:
  ...
  `ALIAS` are used to mimic `CNAME` behaviour, but can also be used on your [apex domain](../apex-domain/) and return the `A` and if configured the `AAAA` records. For more information on why you might use `ALIAS` record over `CNAME` please see this [guide](../alias-record/)
