...
Further information on `mysqldump` and it’s various flags can be found using `man mysqldump` or on the [MySQL documentation site](https://dev.mysql.com/doc/refman/8.4/en/mysqldump.html)
...
Our guide on `cron` can be found [here](../../../operating-systems/linux/basics/cron/), but as an example, here are both the methods laid out as `cron` jobs that would run at 10pm each night:

```
0 22 \* \* _ mysqldump -uroot -pMySuperLongPassword --all-databases > /var/dumps/alldb-`date`.sql
```

```
0 22 _ \* \* innobackupex /var/dumps/
```
...
Once you’re confident that you have consistent backups using one of the above two methods, you should configure your backup exclusions to omit `/var/lib/mysql` (or wherever else you may keep your data directory) from your backup run to save some space.
