...
Full failovers are performed using a Failover Plan which you will have created yourself in your local Veeam Backup and Replication Console. How to create a Failover Plan can be found here - [How to create a Failover Plan](../creating-a-failover-plan/)

A full failover is used in a DR Scenario where you have lost access to your production site. When performing a failover using a Failover Plan the NEA Appliance at ANS is used to configure the External IP NATs that you set up in the mapping section of the Failover Plan configuration. It **does not** set up the Layer 2 VPN connection; this is only possible using partial failovers, which are explained below.

You have the ability to initiate these failovers from within your Veeam Backup and Replication console or via your ANS portal should you not have access to your production site. You also have the option to call ANS support who can initiate the failover for you.

## Partial Failovers

Partial failovers are used to failover individual VMs. Use this type of failover in the event that your production site is still up and running. It will use both the NEA Appliance on yours and ANS's infrastructure to create a Layer 2 VPN tunnel between the sites along with `ProxyARP` solutions enabling VMs to communicate as if they were still in the same Layer 2 network. More information on this can be found here - [NEA Appliance - Partial Failovers](https://helpcenter.veeam.com/docs/backup/cloud/cloud_connect_partial_site_failover.html?ver=100)

## Performing Full Failovers in the Event of an Emergency

### Initiating the Failover

1. Login to your ANS Glass > navigate to the `Disaster Recovery` section down the left hand side > `DRaaS`.
...
![enable failover](../../../backup-availability-images/veeamfailovers_enablefailover.png)

You also have the option to call ANS support, should you have any issues doing this, and we will be able to initiate the failover for you.

#### Undoing the Failover

**This will not replicate changes made at ANS back to your local infrastructure, it will simply power off the VMs at ANS. If you want to replicate changes made at ANS, you must use the "Failback to production" feature on your local Veeam console**
...
![disable failover](../../../backup-availability-images/veeamfailovers_disablefailover.png)

### Test Failover While Production is Still Online

The most likely reason you would do this is to simulate a real DR situation and bring your VMs online at ANS since you have the ability to do this without affecting your live VMs. The VMs will be powered up on ANS's infrastructure and the NEA Appliance will configure the NATs specified in the Failover Plan. Production will not be able to communicate with DR, but you should be able to access your VMs by the NATs you configured with the external IPs.
...
![full failover](../../../backup-availability-images/veeamfailovers_fullfailover.png)
...
![full failover status](../../../backup-availability-images/veeamfailovers_fullfailoverstatus.png)
...
![test failover](../../../backup-availability-images/veeamfailovers_testfailover.png)
...
![partial failover](../../../backup-availability-images/veeamfailovers_partialfailover.png)
...
![partial failover now](../../../backup-availability-images/veeamfailovers_partialfailover_failovernow.png)

Your VMs in production should now be able to communicate with the VMs you have failed over onto the ANS infrastructure.
...
![partial failover failback](../../../backup-availability-images/veeamfailovers_partialfailover_failback.png)
