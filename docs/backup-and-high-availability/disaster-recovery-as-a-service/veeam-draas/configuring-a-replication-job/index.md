---
sidebar_label: "Configuring a Replication Job"
sidebar_position: 4
---

# Configuring a Replication job

Before configuring your replication job please ensure you have read through this document: [Veeam Replication Jobs](../veeam-replication-jobs/)
...
![creating a job](../../../backup-availability-images/createreplicationjob.png)
...
1. Enter a name for the replication job and a description if needed (something that makes it easy to reference which VMs are in the job.)
   ...
   2. **Network remapping** - **(enable)** - Used to map the internal networks on your current VMs to the internal networks ANS has provided to you.
   3. **Replica re-IP** - **(do not enable)** - Used when your DR sites use different internal networks to your production. This is not required when replicating to ANS, as you will keep your current internal IPs.

![replication job - name](../../../backup-availability-images/createreplicationjob_name.png)
...
![replication job - virtual machines](../../../backup-availability-images/createreplicationjob_virtualmachine.png)

### Destination

1. Under "Host or cluster" select `Choose` > `Cloud host` > select the `ANS Service Provider` (ANS's external DNS name with your login name after in brackets) > Click `OK`.
2. Under "Datastore" ensure that your datastore is selected. If you have multiple, select the relevant one.

![replication job - destination](../../../backup-availability-images/createreplicationjob_destination.png)

### Network

Here you'll create your network mappings. When you create a mapping all VMs on the "source network" at your site will be connected to the "target network" at ANS. You should have a dedicated target network for each of the source networks which you are replicating VMs from.

1. Select `Add` to create a network mapping > select the `source network` and `target network`

![replication job - network](../../../backup-availability-images/createreplicationjob_network.png)
...
2. **Replica name suffix** - This shows up within ANS's VMware environment, so we recommend that it is left as the default "\_replica".
...
4. **Advanced**
   1. **Storage optimisation** - This setting needs changing to "WAN Target" to ensure the replication jobs get the best performance when replicating over the internet to ANS
...
![replication job - job settings](../../../backup-availability-images/createreplicationjob_jobsettings.png)

### Data Transfer

Section to configure WAN accelerators, these are only beneficial to use when you have a network upload speed of lower than 60Mb/s. If you have not purchased a WAN accelerator from ANS, you only need to complete step 1 ("Source Proxy"). You cannot configure one at your side if it hasn't been enabled on the service provider's end.
...
2. **Through built-in WAN accelerators** - if you have purchased a WAN accelerator from ANS, select this option.
   ...
   2. **Target WAN accelerator** - select the WAN accelerator at ANS's site to be used for this replication job.

![replication job - data transfer](../../../backup-availability-images/createreplicationjob_datatransfer.png)
...
![replication job - guest processing](../../../backup-availability-images/createreplicationjob_guestprocessing.png)
...
![replication job - schedule](../../../backup-availability-images/createreplicationjob_schedule.png)
