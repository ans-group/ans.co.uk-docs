...
![computer](../../backup-availability-images/computer.png)

You should not use this to add new computers or servers to be backed up - instead follow the instructions shown on [Setting up the FASTcloudbackup agent](../setting-up-the-fastcloudbackup-agent/) - this will guide you through installing the agent so you can run and manage backups.
...
To make it simpler to get to the folders you access regularly, set them up as [favourite folders](../managing-files-and-folders/#favourite-folders).
...
![process manager](../../backup-availability-images/process_mgr.png)
...
![process manager 2](../../backup-availability-images/process_mgr2.png)

You can also check this on Windows computers by opening the `Run` programme and then `services.msc`.

![services msc](../../backup-availability-images/services_msc.png)
...
![services list](../../backup-availability-images/services_list.png)
...
You can set up an alert so you are notified if a single backup job exceeds 24 hours - see [Alerts](../setting-up-alerts/) for details.

You can “kill” a backup job at any point through the FASTcloudbackup web console. When a backup job is in progress, click `Kill` in the Schedules area to stop the backup from running.

![kill command](../../backup-availability-images/kill.png)

## Failed backups

You can see the history and status of your backup jobs in the FASTcloudbackup web console by clicking `Recent Backup Jobs` within the computer or server in question. You’ll see a complete list of jobs, along with the state, number of files and total size of backup. This will enable you to identify any backups that did not run successfully.

![recent jobs](../../backup-availability-images/failed2.png)

Possible backup job states are:

- Completed: the backup job ran successfully.
- Failed to Start: this is most likely because the computer or server was not accessible during the scheduled backup window, potentially in line with the policy you configured when [creating a backup schedule](../setting-up-backups/#creating-a-backup-schedule) - for example specifying that backups should only run if the computer is plugged in to a power source rather than on battery mode.

## Showing live machine data and network share data

There are options to show live machine data and network share data within the FASTcloudbackup web console. You’ll find these under the drop down menu at the top of the My Data screen.

![data search](../../backup-availability-images/date_search-1.png)
...
![credentials log in](../../backup-availability-images/live_machine.png)
...
When installing the FASTcloudbackup agent, a Migration Assistant will also be added to your computer or server. However you should not use this to complete the installation or setup process. Simply follow the instructions in [Setting up the FASTcloudbackup agent](../setting-up-the-fastcloudbackup-agent/).

![migration assistant](../../backup-availability-images/migration_assistant.png)
