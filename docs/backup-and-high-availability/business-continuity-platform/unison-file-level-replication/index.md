---
sidebar_label: "Unison file-level replication"
sidebar_position: 4
---

# Unison file-level replication

In order to keep configuration files and credentials synchronised between all nodes in the cluster, we run a regular [cron](../../../operating-systems/linux/basics/cron/) job to check on the files specified in the Unison configuration.
...
Once this file has been saved, it will take effect from the next run of the [cron](../../../operating-systems/linux/basics/cron/) job, which should be configured similar to the following as the `root` user:
