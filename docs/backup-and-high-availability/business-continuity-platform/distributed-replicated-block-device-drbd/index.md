---
sidebar_label: "Distributed Replicated Block Device (DRBD)"
sidebar_position: 3
---
...
DRBD is active / passive by nature, so only one node can be “primary” for the volume at a time - and therefore only one node can have the volume mounted at a time. If more than one node considers itself “primary”, the cluster has likely become [split brain](../split-brain/).
