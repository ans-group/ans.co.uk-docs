...
ANS close the above TCP/UDP ports with our default Firewall configuration, these ports may be open if requested manually or manually changed via the [ANS Glass](https://ans.glass) Firewall Portal.
...
ANS recommend this security patch be applied with immediate effect if not already installed on your Windows platform. Upon installation of the security patch will require a server restart, we advise taking note of any highly available (HA) services and/or clusters when performing restarts. If you are unfamiliar with this process or require some assistance please contact our support team either via [ANS Glass](https://ans.glass/support-and-help/incidents), by emailing [support@ans.co.uk](mailto:support@ans.co.uk) or by calling 0800 230 0032.
...
- ANS's default firewall configuration excludes access on the above TCP and UDP ports for public access, however these ports can be modified by your team(s) to be open for public access. To clarify if your firewall has these ports open or not please login to the [ANS Glass](https://ans.glass) firewall section to verify the firewall port configuration. ANS recommends the SMB protocol TCP and UDP ports be closed for public access if open, we do not recommend allowing public access to the SMB protocol as standard practice. If you are unfamiliar with firewall ports or require additional assistance, please contact our support team on any of the following:

  - [Priority Support Request](https://ans.glass/support-and-help/incidents)

  - Email [support@ans.co.uk](mailto:support@ans.co.uk)

  - Telephone 0800 230 0032

- ANS recommends a review of domain and local administrative accounts. Allowing end users to have administrative access is quite common when faced with permission related problems. We do not recommend allowing user accounts to have domain or local administrative access.
...
ANS recommends upgrading the operating system at your earliest convenience. Windows 2003 and XP are no longer within support scope for standard patching and will be subjected to a number of zero-day exploits which will leave your server at risk.

### Paying for decryption?

ANS cannot endorse the payment of ransomware to third parties. This decision will require serious deliberation within your company to choose the desired option for your business.

## Contacting ANS Support
