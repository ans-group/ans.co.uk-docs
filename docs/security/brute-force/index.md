...
- Using a [Web Application Firewall](../web-application-firewall/) or security plugin such as [Wordfence](https://www.wordfence.com/).
- For server-level services such as SSH and FTP, we'd recommend locking down the ports in your [firewall](../../network/firewalls/) so that only you can access them. This removes the attack vector entirely as they can no longer attempt to login.
